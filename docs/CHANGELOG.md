...
### Added
- **PDF Compare (beta).** A new card before PDF → Word and PDF → PowerPoint compares an earlier born-digital PDF on the left with a later one on the right, without converting either document. Original pages stay side by side; only authoritative earlier/left removals receive tight solid red (`#EC0808`) word backgrounds and only later/right additions receive tight solid green (`#1BE91A`) word backgrounds. Fills never bridge spaces, unchanged text or geometric gaps; dark original glyphs remain readable and chromatic source ink is preserved. Persistent localized `−`/`+` side legends retain ownership, while Windows High Contrast deliberately uses system colours, outlines and patterns rather than custom red/green fills. Each pane is read-only and has an independent trusted selection: select words with the mouse, use Ctrl+A for the current page, and Ctrl+C to copy Unicode text for pasting into another application. Copied words come only from the final published PDF text layer; separators use literal decoded source evidence, with one disclosed readability space only where a trusted boundary is unavailable—never OCR, raster pixels or geometry, and never semantic whitespace evidence. Separate `␠`/`NBSP`/`⇥`/`↵` markers likewise identify only literal text-layer whitespace. Bitmap and trusted-text publication is atomic and revision-checked, so loading, replacement, missing/unavailable pages and stale render completions cannot leave an old selectable layer attached. The alignment-row list exposes changed, inserted, deleted and reordered pages with previous/next-change navigation and optional manual pairing. Image-only/scanned PDFs remain out of scope (no OCR); a protected PDF uses the existing in-memory password prompt instead of being rejected. Default guardrails are 200 MB per file, 500 pages, 2 M extracted characters, 25 M render pixels, 1 M exact-diff cells and a finite total semantic-work budget, with four successful extractions cached in memory.
