...
### Added
- **PDF Compare (beta).** A new card before PDF → Word and PDF → PowerPoint compares an earlier born-digital PDF with a later one without converting either document. Its default unified redline uses the later page as the base, keeps additions green, and overlays only deleted earlier-version fragments in red with a strike line; unchanged earlier-page pixels never cover the result. The original pages remain available side by side with one click, and changing view never reruns the semantic comparison. Double-click opens the active canvas maximized with the exact same markup, while the alignment-row list has a visible user-draggable splitter. Only authoritative earlier removals receive tight solid red (`#EC0808`) word backgrounds and only later additions receive tight solid green (`#1BE91A`) word backgrounds. Dark original glyphs remain readable and chromatic source ink is preserved. Windows High Contrast deliberately uses system colours, outlines and patterns rather than custom fills. Each source pane is read-only and has an independent trusted selection: select words with the mouse, use Ctrl+A for the current page, and Ctrl+C to copy Unicode text. Copied words come only from the final PDF text layer; separators use literal decoded source evidence, with one disclosed readability space only where a trusted boundary is unavailable—never OCR, raster pixels or geometry. Separate `␠`/`NBSP`/`⇥`/`↵` markers likewise identify only literal text-layer whitespace. Bitmap and trusted-text publication is atomic and revision-checked, so loading, replacement, missing pages and stale render completions cannot attach an old selectable layer. Image-only/scanned PDFs remain out of scope (no OCR); a protected PDF uses the in-memory password prompt. Guardrails are 200 MB per file, 500 pages, 2 M extracted characters, 25 M render pixels, 1 M exact-diff cells and a finite semantic-work budget, with four successful extractions cached in memory.
- **A calm, version-aware “What’s New” window.** The current version's short bilingual notes are embedded in the executable and appear once, modelessly, after the hub is available—not as a startup-blocking dialog. “Don't show this automatically again” is unchecked unless previously chosen; Settings can re-enable automatic display and always has a “What's new in 1.18.5” button. A muted independent-development support link sits below the feature cards, with bank details collapsed until explicitly opened.
...
### Changed
- **PDF comparison is document-wide, not page-local.** Canonical visible words from both files are flattened in physical order and compared exactly once; page alignment exists only for viewer rows, tie corroboration and highlight projection. Moving the same text across page boundaries therefore creates no edit, while real insertion/deletion/replacement remains visible. Typographic styling, colour, coordinates and page geometry do not enter word equality. Every operation now retains its actual earlier/left and later/right counterparts plus match provenance, and manual physical-page pairing never launches a second semantic diff. The exact LCS has a paired-page secondary tie preference for repeated content without sacrificing LCS length; large middles use deterministic bounded-memory Myers bisect, and exhausted proof work keeps conservative Delete/Insert candidates.
- **Form/table extraction order and literal whitespace now have separate conservative proofs.** A bounded post-diff reconciliation removes an order artifact only within one explicit physical-page pair when exact NFC key multiplicity and local geometry admit one unique matching; repeated-anchor ambiguity, real movement, cross-page material or nearby unresolved edits abstains. `foobar` ↔ `foo bar` becomes a split/join equivalence only with exact source spans, stable anchors, Unicode-safe boundaries and matching flow/geometry. U+0020 multiplicity, NBSP, tabs, logical CR/LF and other Unicode whitespace are compared separately only when decoded text-layer evidence—including a positively proven empty boundary—exists on both sides. Visual gaps, reconstructed rows, wrapping, page edges and raster pixels never manufacture whitespace. Whitespace-only edits can change page status, but have separate counters and do not alter word replacements or the word-based percentage.
- **Review state publication and cache invalidation follow the new semantics.** Projection is the only builder of word/whitespace page indexes, row status and statistics; it validates and atomically publishes the complete snapshot, so cancellation, work exhaustion or a failed refinement cannot expose stale or partially rebuilt state. The private extraction-cache discriminator is now `review-8`; the public release remains 1.18.5.
- **Raster confirmation is bounded, word-only and strictly subtractive.** It may suppress only a pre-existing mixed Delete/Insert word hunk wholly owned by one explicitly paired physical-page row and proven equivalent at useful bounded resolution; it cannot create a match, reinterpret whitespace or turn a raster match into a trusted whitespace anchor. Shared candidate/relation/sample-pixel ceilings bound the whole result, every render bitmap has explicit disposal ownership, and publication verifies that its semantic source snapshot is still current. Pure insertion/deletion, cross-page ownership, missing pairing, renderer failure, cancellation, cap exhaustion, ambiguous/no ink or an inadequate render retains the text-semantic candidate. Deterministic temporary PDFs exercise the production route in both directions, including re-pagination, two-dimensional extraction order, exact whitespace and equivalent low-level representations; the untouched local `1.pdf`/`2.pdf` fixtures are also checked when present.
- **PDF Compare source and viewer interactions are now explicit.** Either path can be typed or pasted and is validated asynchronously; browse, swap and drag-and-drop use the same source lifecycle. Two dropped PDFs fill left/right in order, a single PDF over a side replaces that side, a neutral single-file drop fills the first empty side, and an ambiguous or oversized drop is explained instead of overwriting silently. Ordinary wheel input scrolls only the pane beneath the pointer, then continues at its page boundary to the nearest alignment row containing a page on that same side, skipping gaps without wrapping; downward movement opens at the top and upward movement at the bottom. Ctrl+wheel zooms only the hovered pane around the pointer and never changes the row.
...
### Fixed
- **The refined Compare UI now exposes every supported change without fighting the reader.** Unified view carries both red removed-whitespace and green added-whitespace markers, not just word edits. Text selection accepts the natural white space immediately around a line and continues across gaps instead of requiring a pixel-perfect word hit. Side-by-side view keeps only compact colour legends above the pages, leaving substantially more room for the PDF at the default and minimum window sizes.
- **The “What’s New” window opens where expected and keeps its reading position.** The automatic modeless window is centred on its owner, its first frame starts at item 1 rather than a stale blank scroll offset, and expanding voluntary support details reveals them by scrolling only downward—never jumping back to the top.
- **PDF Compare no longer shows an empty white page before rendering or loses drops over nested controls.** Each viewer now has explicit empty, drop-target, loading, ready, missing-counterpart and unavailable states; the page canvas exists only for a ready bitmap, drop routing follows the actual child under the pointer, and stale asynchronous source/render completions are rejected during edits and shutdown.
...
- **Atomic replacement now survives the failure of its own rollback.** A full-name recovery backup and live `.txn` journal accompany the non-NTFS rename fallback; neither `Dispose` nor startup cleanup may delete the only surviving original. Settings, counters and history distinguish a missing file from an unreadable one, serialize updates with a cross-session file lock, preserve unknown future keys and never report a swallowed write as successful. History opt-out and shorter retention notify the hub and remove paths immediately; the installer language marker is consumed only after its setting is durably committed.
- **PDF raster memory is admitted before allocation and owned by single-release leases.** Document/page/tile caches evict and retry instead of stalling, x86 uses lower pixel/DPI limits, every dimension is bounded, late page renders are rejected during replacement/shutdown, and the Compare viewer remains truly single-flight. PowerPoint backgrounds cap pixels, temporary bytes and ranges, deduplicate repeated pages, retain thin graphics through an exact solid-colour check, support password-opened sources through a cleaned decrypted working copy, and terminate Ghostscript on cancellation/timeout.
- **Image and command-line failure paths are transactional and honest.** Image export reserves collision-free names and atomically publishes each page, a failed encoder cannot leave a truncated file, large/multipage image input has aggregate memory admission and per-frame cancellation, and output-write errors no longer masquerade as unreadable input. CLI rejects irrelevant/conflicting/duplicate options, accepts `--` before dash-prefixed paths, returns bad-usage for a mistyped command instead of opening the GUI, and distinguishes a valid “not smaller” compression result from engine failure.
- **Long filesystem work no longer accumulates on the UI path.** Excel folder scans are latest-wins and single-flight, stale scans cannot repopulate the list, and closing the window performs no network existence probe. Recovery sweep runs after the shell appears rather than blocking startup on an offline path.
- **Release and attribution checks are now explicit.** PDF Compare appears in local statistics; PDFsharp/PdfPig licenses and notices are embedded and available from About; CI exercises first-touch embedded resolution in x64/x86; the release script refuses a dirty tree or tag/asset commit mismatch and requires signed, timestamped artifacts.
