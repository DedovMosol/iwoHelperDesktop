...
### Added
- **Compare two versions of a PDF side by side.** A new tool card in the PDF section: pick a left and a right file, and the window shows the pages in the original view, side by side, with the differences highlighted right on them — removals in red on the earlier version, additions in green on the later one (page-pair list with previous/next navigation and manual pairing). The status-line counters are computed from the very same word-level markup that is drawn, so what is highlighted is exactly what is counted. Digital PDFs only: scanned pages (no text layer) and password-protected files are refused with an explanation instead of a meaningless diff. Guardrails are checked before any heavy work — 200 MB per file, 500 pages, 2 M characters, 1 M diff cells — and a successful extraction stays in an in-memory LRU cache (4 documents), so a second look at the same pair is instant.
