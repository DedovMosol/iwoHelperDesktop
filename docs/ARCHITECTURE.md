...
1. **Unit tests** — `tests/UnitTests.cs` (~230 tests, custom exe runner, zero
   ...
   the drop target, and the single-instance name is taken once and freed on exit. Two
   tests build a real window on an STA thread — the About window (its description must be
   selectable, mention born-digital PDFs and not be clipped, the copyright must sit at the
   bottom-left) and every window in English (no stray Cyrillic, read-only text boxes
   included, since those are labels the user can copy).
