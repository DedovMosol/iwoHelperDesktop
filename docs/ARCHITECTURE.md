...
| `PdfReviewForm.cs`, `PdfReviewPageView.cs` | The **beta PDF Compare** window and its one page surface: editable left/right source fields, pair list, previous/next-change navigation, manual pairing, and the two original pages side by side. The page view has explicit `Empty` / `DropTarget` / `Loading` / `Ready` / `MissingCounterpart` / `Unavailable` states; its white canvas exists only for a ready bitmap. A result content revision joins page identity and render generation, so the raster and matching final page model publish atomically and stale work cannot restore an old selectable layer. Rendering is background and pane-owned. One form-level `IMessageFilter` routes normal wheel scrolling or cursor-anchored Ctrl+wheel zoom only to the viewport under the pointer and is removed on close. |
...
1. **Unit tests** — `tests/UnitTests.cs` (511 tests, custom exe runner, zero
   ...
   **minimized**, whose placement must come back on-screen rather than at the off-screen
   ...
   registered test count to equal 511 exactly, so adding or deleting a `Run(...)` line must be
