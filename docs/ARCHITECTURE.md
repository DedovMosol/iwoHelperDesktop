...
| `ShellContext.cs` | `ApplicationContext` that owns the hub and all tool windows (independent, non-modal). Reopens the hub, focuses an already-open tool, rebuilds windows on language change (active window last, busy windows deferred), exits when the last window closes. Owns the `SingleInstance` listener (created before the first window, so a launch that follows immediately still finds it) and answers the signal with `ShowHub`, which is idempotent. |
...
1. **Unit tests** — `tests/UnitTests.cs` (~230 tests, custom exe runner, zero
   ...
   Ghostscript presets, the preview closes on the left button only, the rotation delta
   lands on the wanted angle from any starting angle, drag hints stay imperative and name
   the drop target, and the single-instance name is taken once and freed on exit. Three
   tests build real windows on an STA thread — the About window (its description must be
   selectable, mention born-digital PDFs and not be clipped, the copyright must sit at the
   bottom-left, checked in both languages because the texts wrap differently), every window
