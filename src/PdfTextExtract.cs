...
namespace ExcelMerger
{
    ...
    public static class PdfTextExtract
    {
            ...
            try
            {
                using (UglyToad.PdfPig.PdfDocument doc = UglyToad.PdfPig.PdfDocument.Open(path))
                {
                    ...
                    foreach (UglyToad.PdfPig.Content.Page page in doc.GetPages())
                    {
                        ...
                        // пунктов теряет прочерки и разваливается.
        ...
        /// слова-прочерки «____»: линии-заполнители пунктов («______ № ______», «На № … от …»)
        /// иначе теряются вовсе, и строка пунктов разваливается. Число «_» подбирается по
        ...
        /// <summary>Слово целиком из «_» (прочерк-заполнитель пунктов)?</summary>
