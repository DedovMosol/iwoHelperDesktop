...
{
    /// <summary>Найденная штамп: рамка (PDF, ось Y вверх) и слова, которые она накрывает.</summary>
    ...
    /// <summary>
    /// Распознавание текстового штампа, вписанного в PDF ТЕКСТОМ (а не картинкой):
    /// «Документ с текстовым штампом / Сертификат … / Владелец … / Действителен …».
    ...
    ///
    /// Детекция НАРОЧНО строгая: нужны все четыре опорных слова (подписан, сертификат, владелец,
    /// действителен) в компактной вертикальной полосе. Отсюда ключевое свойство: ложные
    /// срабатывания практически исключены (в обычном тексте эти слова не стоят кучно в рамке), а
    /// если штамп не распознан — поведение прежнее (текст остаётся текстом), то есть регрессии нет.
    /// Растровые печати (нарисованные картинкой) сюда не попадают — у них нет извлекаемого текста,
    /// их и так переносит ExtractImages. Чистая логика без типов PdfPig — под юнит-тест.
    /// </summary>
    internal static class StampDetector
    {
        ...
        /// <summary>
        /// Найти текстовый штамп среди слов страницы. Возвращает область и её слова, либо null.
