...
{
    /// <summary>Найденный текстовый штамп: рамка (PDF, ось Y вверх) и слова, которые он накрывает.</summary>
    internal sealed class StampRegion
    {
        ...
        public double Top;
        public List<PdfWord> Words = new List<PdfWord>(); // слова штампа — их снимаем из текста (штамп станет картинкой)
    ...
    /// Такой штамп при конвертации иначе растекается обычными строками и теряет вид штампа —
    ...
    /// прежнее (текст остаётся текстом), то есть регрессии нет. Растровые штампы (нарисованные
    ...
    internal static class StampDetector
    {
        ...
        // Компактность итоговой рамки (с содержимым строк — номер, ФИО, даты).
        ...
        private const int MinStampWords = 6;         // штамп — минимум несколько слов (заголовок + поля)
