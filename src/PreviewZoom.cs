...
namespace ExcelMerger
{
    ...
    public static class PreviewZoom
    {
        ...
        /// <summary>
        /// Где стоит картинка в прокручиваемой области: по центру, когда она меньше окна, и в
        /// начале, когда больше. Смещение прокрутки прибавляется КАК ЕСТЬ — оно читается
        /// отрицательным, а положение дочернего элемента WinForms хранит в координатах
        /// СОДЕРЖИМОГО, а не клиентской области.
        ///
        /// Без этой поправки начало содержимого уползало на величину прокрутки при каждом шаге
        /// лупы и накапливалось: страница оказывалась ниже и правее видимой области, окно
        /// показывало пустой серый фон, и её приходилось искать прокруткой. Чистая — под тест.
        /// </summary>
        public static Point Centered(Size scaledImage, Size viewport, Point scrollOffset)
        {
            return new Point(
                Math.Max(0, (viewport.Width - scaledImage.Width) / 2) + scrollOffset.X,
                Math.Max(0, (viewport.Height - scaledImage.Height) / 2) + scrollOffset.Y);
        }
