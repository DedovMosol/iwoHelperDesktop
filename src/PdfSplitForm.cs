...
namespace ExcelMerger
{
    ...
    public class PdfSplitForm : PdfSingleDocFormBase
    {
            ...
            CompressionLevel level, long sourceSize, long largestOutput, int pageCount)
        {
            ...
            // В истории у разделения ОДИН путь: файл при извлечении и папка при разбиении на
            // части — открывать всё равно нужно именно его, а перечислять десяток частей
            // отдельными строками значило бы утопить в них остальные операции.
            OperationHistory.Record(openAsFolder ? "hist.op.split" : "hist.op.extract", openTarget);
            if (compressed > 0)
