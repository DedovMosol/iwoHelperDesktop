...
namespace ExcelMerger
{
    ...
    internal static class PdfColorProbe
    {
        ...
        /// <summary>
        /// Ширина рендера. Цвет виден и на маленькой картинке, а памяти и времени просит мало.
        /// Высоту не ограничиваем, хотя PDF разрешает лист до 14400 пунктов при ширине в
        /// единицы: на такой странице пропорциональная высота ушла бы в сотни тысяч пикселей.
        /// Проверено на предельном листе 3×14400 — системный рендер за такое просто не
        /// берётся и отдаёт пусто (102 мс, ноль дополнительной памяти), а пустой ответ у нас
        /// по построению означает «сказать нечего», не «остался цвет». Городить расчёт
        /// ограничения не на что.
        /// </summary>
        ...
        public static bool HasColorPages(string path)
        {
            ...
            using (var renderer = new PdfThumbnailRenderer())
            {
                ...
                {
                    using (Bitmap bmp = renderer.Render(path, index, RenderWidth))
                        if (HasColor(bmp))
                            return true;
                }
            }
