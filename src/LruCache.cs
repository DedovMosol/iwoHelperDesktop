...
namespace ExcelMerger
{
    ...
    ///
    /// НЕ потокобезопасен: вызывающий обеспечивает доступ из одного потока. В приложении
    /// оба кэша однопоточны, но потоки РАЗНЫЕ: кэш отрендеренных страниц PdfPageGrid живёт
    /// целиком на UI-потоке, кэш документов PdfThumbnailRenderer — на потоке рендера
    /// миниатюр. Логика вытеснения чистая и покрыта юнит-тестами.
    /// </summary>
