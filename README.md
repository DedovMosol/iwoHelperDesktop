...
[![Installer x64](https://img.shields.io/badge/Installer-x64-0F6CBD?style=for-the-badge&logo=windows&logoColor=white)](https://github.com/DedovMosol/iwoHelperDesktop/releases/download/v1.17.5/iwoHelperDesktop-setup-1.17.5.exe) [![Portable x64](https://img.shields.io/badge/Portable-x64-107C41?style=for-the-badge&logo=windows&logoColor=white)](https://github.com/DedovMosol/iwoHelperDesktop/releases/download/v1.17.5/iwoHelperDesktop-1.17.5.exe) [![Installer x86](https://img.shields.io/badge/Installer-x86-0F6CBD?style=for-the-badge&logo=windows&logoColor=white)](https://github.com/DedovMosol/iwoHelperDesktop/releases/download/v1.17.5/iwoHelperDesktop-setup-1.17.5-x86.exe) [![Portable x86](https://img.shields.io/badge/Portable-x86-107C41?style=for-the-badge&logo=windows&logoColor=white)](https://github.com/DedovMosol/iwoHelperDesktop/releases/download/v1.17.5/iwoHelperDesktop-1.17.5-x86.exe)
...
- 📄 **PDF Merge** — build one PDF from several: a thumbnail grid with page numbers and zoom up to 400 px **shown as an editable percentage** (type an exact value in the “%” box, or Ctrl+wheel over the slider or the grid, or Ctrl+0 to reset to 100%), **double‑click a page for a full‑size preview** (or double‑click its number to move the page), reorder by dragging (the grid auto‑scrolls at the edges), **cut/copy/paste pages** (Ctrl+X/C/V) with an insertion caret, **Move after page N…** for exact long jumps, and Ctrl+Z / Ctrl+Y to undo and redo edits (rotations included). **Rotate selected pages or the whole document 90°** (right‑click menu, Ctrl+Shift+«+»/«−», or the ↺/↻ buttons that appear on the tile under the cursor — the tile shows a rotation badge), jump to a page with Ctrl+G, and **drop more PDFs straight onto the grid** — their pages land at the drop position. Pages are copied **as‑is** — scans, stamps and signatures are not distorted (rotation is written as page metadata). **A long save can be cancelled** (a Cancel button appears for documents of five pages or more), and the **last zoom and compression level are remembered** between runs (as is each window's size and position) — shared by Split and PDF → Word.
...
- 🌐 **English & Russian interface** — the installer opens with a flag chooser (Russian or English), runs the wizard in that language and makes it the app's default, so an English user is never shown Russian (the portable build follows the system language on first run). You can switch anytime from a globe button on the start screen or any tool’s **☰ Menu → Язык / Language** (each option shows a country flag). The choice is saved and applied instantly, open windows rebuild in the new language on the spot. Generated documents (the cover note and reports) stay in Russian.
...
| Windows | Download |
|----|----------|
| **64‑bit** — Windows 8.1 / 10 / 11 *(most PCs)* | [![Installer](https://img.shields.io/badge/Installer-x64-0F6CBD?logo=windows&logoColor=white)](https://github.com/DedovMosol/iwoHelperDesktop/releases/download/v1.17.5/iwoHelperDesktop-setup-1.17.5.exe) &nbsp; [![Portable](https://img.shields.io/badge/Portable-x64-107C41?logo=windows&logoColor=white)](https://github.com/DedovMosol/iwoHelperDesktop/releases/download/v1.17.5/iwoHelperDesktop-1.17.5.exe) |
| **32‑bit** — 32‑bit editions of Windows 8.1 / 10 | [![Installer](https://img.shields.io/badge/Installer-x86-0F6CBD?logo=windows&logoColor=white)](https://github.com/DedovMosol/iwoHelperDesktop/releases/download/v1.17.5/iwoHelperDesktop-setup-1.17.5-x86.exe) &nbsp; [![Portable](https://img.shields.io/badge/Portable-x86-107C41?logo=windows&logoColor=white)](https://github.com/DedovMosol/iwoHelperDesktop/releases/download/v1.17.5/iwoHelperDesktop-1.17.5-x86.exe) |

*The buttons download the current release (**v1.17.5**) directly, each file name carries its version. The newest build and the full file list are always on the [releases page](https://github.com/DedovMosol/iwoHelperDesktop/releases/latest).*
...
- **Portable** — a single `iwoHelperDesktop-1.17.5.exe` (`…-x86.exe` for 32‑bit) — just run it. PDF compression works if Ghostscript is installed on the machine.
