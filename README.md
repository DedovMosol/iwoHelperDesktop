...
- 📝 **PDF → Word** — extract the text layer of one or **several born‑digital** PDFs (saved from Word, “Microsoft Print to PDF”, exported from a browser) into a single editable `.docx`. Inherits the font family, size, bold/italic, underline, colour, super/subscript, paragraph alignment (left/justify/centre) and first‑line indent, per‑page size and orientation (portrait/landscape) and margins, images (placed in reading order) and hyperlinks. **Bordered tables are reconstructed as real Word tables** — column widths from the ruling, merged cells (colspan/rowspan), and per‑cell text. Add several PDFs at once; every file’s pages appear in one thumbnail grid where you can reorder by dragging (or ◀ Раньше / Позже ▶) and drop pages you don’t need, so Word gets them — from all files — in exactly the order shown. If a font used in the PDF is not installed, the text is set in Times New Roman; tables **without** ruled borders, multi‑column layouts and lists are flattened to single‑column paragraphs, and electronic‑signature seals are not redrawn as graphics (their certificate text is still extracted). Scanned documents are not supported yet — a clear message is shown and the file is untouched.
