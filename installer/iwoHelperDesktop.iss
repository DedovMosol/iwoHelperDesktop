...
VersionInfoCopyright=© 2026 {#Publisher}. GNU General Public License v3.0 only.
...
// Убрать за собой временную папку setup ПОСЛЕ выхода. ExitProcess обрывает процесс, минуя
// штатную уборку Inno, поэтому папка с распакованными флагами оставалась в %TEMP% после
// каждой смены языка. Поручаем уборку отдельному cmd: он ждёт пару секунд (ping — переносимая
// пауза, timeout без консоли не работает) и удаляет папку, когда файлы уже освобождены.
// Отдельной командой, а не довеском к перезапуску: сбой уборки не должен сорвать сам
// перезапуск, который и есть главное. Неудача уборки не страшна — будет как раньше.
procedure CleanTempAfterExit();
var
  ErrCode: Integer;
begin
  Exec(ExpandConstant('{sys}\cmd.exe'),
    '/c ping -n 3 127.0.0.1 >nul & rd /s /q "' + ExpandConstant('{tmp}') + '"',
