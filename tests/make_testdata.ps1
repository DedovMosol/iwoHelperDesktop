...
try {
    ...
    # формулы со строковыми результатами: при замене значениями Excel не должен
    # заново разбирать строку (формула-инъекция, число, дата)
    ...
    $wb.SaveAs((Join-Path $dir "Очень длинное имя файла отчета организации за март 2026.xlsx"), 51)
    $wb.Close($false)
